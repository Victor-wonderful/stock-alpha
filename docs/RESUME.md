...
1. **밤새 돌린 8셋업 백테스트 결과 확인** (기존 4 + 신규: 수급매집·눌림목·52주신고가·변동성수축)
   ```sql
   select setup, expectancy_r, mdd, win_rate, passed from backtests
   where created_at = (select max(created_at) from backtests) order by setup;
   ```
   - 첫 시도는 0원 가격(거래정지 이력) ZeroDivision 으로 중단 → 가드 추가 후 재실행 걸어둠(11일 새벽). 결과 없으면 `engine.cli backtest` 재실행.
   - 횡단면(factor_composite)은 재확인 완료: **FAIL 유지**(IC 0.043 유효·톱데실 초과수익 t=-0.04).
   - 600일 이력 기준 첫 검증이라 기존 3종(주도주/돌파/종가베팅) 판정도 바뀔 수 있음 — 바뀌면 그게 더 신뢰할 수치(표본 2.4배).
2. **통과 셋업으로 시그널 재생성**: `delete from signals;` → `engine.cli signals --setups "<통과목록>"` (또는 그냥 `daily --skip-ingest`로 전체 체인). 픽 재선정 포함.
3. **스크리너 필터 갱신**: `apps/web/components/ScreenerFilters.tsx` ACTIVE_SETUPS = 통과 셋업만 (유령 필터 금지 원칙).
...
5. 검증 페이지(/strategies)에서 신규 셋업 표시 확인 → 커밋 → 이후 아래 백로그 진행.
