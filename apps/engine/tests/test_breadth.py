...
def test_breadth_condition():
    ...
    hi = CONDITIONS["열 종목 중 일곱 이상이 올랐습니다"]
    lo = CONDITIONS["열 종목 중 일곱 이상이 내렸습니다"]
...
def test_trend_condition_needs_20_days():
    ...
    up = CONDITIONS["최근 한 달 시장이 5% 넘게 올랐습니다"]
...
# ── 서술 — 기준선을 반드시 함께 ────────────────────────────────────────
def test_sentence_always_includes_baseline_and_counts():
    c = {"condition": "사흘 연속 올랐습니다", "n": 86, "sample_1d": 86,
         "up_count_1d": 58, "up_rate_1d": 0.674}
    out = _condition_sentence(c, {"up_rate_1d": 0.553})
    assert "86번" in out          # 표본 수
    assert "58번" in out          # 비율이 아니라 횟수로 — 사람이 세는 단위
...
def test_sentence_calls_out_no_edge():
    # 기준선과 3%p 미만 차이는 '별 차이 없다'고 말한다 — 없는 엣지를 있는 척 안 함.
    c = {"condition": "최근 한 달 시장이 5% 넘게 올랐습니다", "n": 93,
         "sample_1d": 93, "up_count_1d": 54, "up_rate_1d": 0.576}
    out = _condition_sentence(c, {"up_rate_1d": 0.553})
    assert "별 차이가 없습니다" in out


def test_sentence_derives_count_when_missing():
    # 옛 payload(횟수 없음)도 비율에서 역산해 문장이 깨지지 않는다.
    c = {"condition": "사흘 연속 올랐습니다", "n": 86, "up_rate_1d": 0.674}
    out = _condition_sentence(c, {"up_rate_1d": 0.553})
    assert "58번" in out
...
def test_fallback_uses_market_numbers():
    mk = {
        ...
        "conditions": [{"condition": "사흘 연속 올랐습니다", "n": 86,
                        "sample_1d": 86, "up_count_1d": 58, "up_rate_1d": 0.674}],
    }
    b = fallback_brief(_ctx(mk))
    assert "1,457" in b["headline"] and "891" in b["headline"]
    assert "86번" in b["market_view"] and "55%" in b["market_view"]
...
def test_fallback_says_nothing_special_when_no_condition():
    ...
    b = fallback_brief(_ctx(mk))
    assert "뚜렷하게 다른 점은 없는" in b["market_view"]
