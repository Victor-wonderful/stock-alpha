...
// ── 랜딩(비로그인 홈) 숫자 ──
/**
 * 처음 온 사람에게 보여 줄 **셀 수 있는 사실**만.
 *
 * ⚠️ 여기에 성과(승률·수익률)를 쓰지 않는다(2026-08-24 Victor 확정). 화면에 남아 있는
 * 성적은 폐기된 옛 규칙의 것이고, 목표 도달은 30건 중 0건이다 [[pick-target-never-reached]].
 * 검증되지 않은 수치를 «광고 문구»로 한번 내보내면 되돌릴 수 없다. 그래서 랜딩은
 * 자랑을 하지 않고 **오늘 이 기계가 실제로 한 일의 양**만 말한다.
 *
 * 셋 다 count 만 세는 head 쿼리라 행을 한 줄도 실어 오지 않는다 — 종목명·가격은
 * 로그인 뒤에만 나간다.
 */
export interface LandingStats {
  /** 기준일에 발행된 픽 수 */
  publishedToday: number;
  /** 아직 들고 있는(status=open) 픽 수 */
  tracking: number;
  /** 기준일에 나온 종목 분석 리포트 수 */
  reports: number;
  /** 분석 기준일 — «오늘»은 마지막 거래일이다 [[basis-day-not-calendar-today]] */
  asOf: string | null;
}

export async function getLandingStats(): Promise<LandingStats> {
  const empty: LandingStats = {
    publishedToday: 0, tracking: 0, reports: 0, asOf: null,
  };
  try {
    const supabase = createPublicClient();
    const asOf = await getLatestReportDay();

    const [picks, open, reports] = await Promise.all([
      asOf
        ? supabase
            .from("recommendations")
            .select("id", { count: "exact", head: true })
    ...
    return {
      publishedToday: picks.count ?? 0,
      tracking: open.count ?? 0,
