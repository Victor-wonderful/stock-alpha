...
export function exitPlanLines(
  ...
  if (tp1 != null) {
    lines.push({
      // 「목표가」라는 이름을 버렸다(2026-08-22 Victor). 파는 값이 아닌데 그렇게
      // 읽히기 때문이다 — 실제로 하는 일은 손절을 본전으로 올리는 것 하나뿐이다.
      trigger: h ? `본전 도달가 ${fmt(tp1)}` : `목표 ${fmt(tp1)} 도달`,
      action: h
        ? "팔지 않고 손절을 본전으로 올림 — 되돌림만 막고 상방은 안 자름"
