...
/**
 * 랜딩 — **로그인하지 않은 사람이 보는 유일한 화면**(2026-08-24 Victor 확정).
 *
 * 홈을 뺀 전 화면이 회원 전용이 되면서, 지금까지의 홈(상태판)을 그대로 두면 잠금이
 * 무의미해진다 — 그 화면에 이미 오늘의 픽 표·리포트·뉴스가 다 깔려 있었다. 그래서
 * 같은 `/` 경로가 **로그인 여부로 두 얼굴**을 갖는다.
 *
 *   비로그인  이 화면 — 여기가 뭐 하는 곳인지, 오늘 얼마나 돌았는지
 *   로그인    기존 홈(app/page.tsx) — 오늘 무슨 일이 있었나
 *
 * 지켜야 할 두 가지:
 *
 * 1. **성과 숫자를 쓰지 않는다.** 「승률 ○○%」를 넣고 싶어지는 자리인데, 화면에 남은
 *    성적은 폐기된 옛 규칙의 것이고 목표 도달은 30건 중 0건이다. 검증 안 된 수치를
 *    광고로 한번 내보내면 되돌릴 수 없다. 대신 셀 수 있는 사실만 — 오늘 발행 건수,
 *    추적 중 건수, 분석 리포트 수.
 * 2. **없는 기능을 약속하지 않는다.** 관심 종목·내 픽 추적·알림은 아직 만들지 않았다.
 *    「회원이 되면」에는 **오늘 실제로 열리는 것**만 적는다. 기능이 생기면 그때 늘린다.
 *
 * GNB(8개 메뉴)를 쓰지 않는 이유: 여기서는 모든 메뉴가 벽이다. 눌러 봐야 로그인 화면으로
 * 튕기는 버튼 여덟 개를 첫 화면에 두면 그게 첫인상이 된다. 머리에는 로그인·회원가입만 둔다.
 */
